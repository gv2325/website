  ...
  return (
    <nav className="flex w-full items-center bg-[#E9E4DD] text-[#2E090D] lg:min-h-18 lg:px-[5%]">
      <div className="mx-auto size-full lg:grid lg:grid-cols-[0.375fr_1fr_0.375fr] lg:items-center lg:justify-between lg:gap-4">
        <div className="flex min-h-16 items-center justify-center px-[5%] md:min-h-18 lg:min-h-full lg:px-0">
          <a href={logo.url}>
            <Image src={logoTxt.src} alt={logo.alt || "Logo"} width={logoTxt.width || 100} height={logoTxt.height || 50} />
          </a>
          <div className="px-0.5"></div> {/* Added padding between images */}
          <a href={logo.url}>
            <Image src={logo.src} alt={logo.alt || "Logo"} width={logo.width || 100} height={logo.height || 50} />
          </a>
          <div className="flex items-center gap-4 lg:hidden">
            <div>
              {buttons.map((button, index) => (
                <Button key={index} className="w-full px-4 py-1" style={{ backgroundColor: '#781E26' }} {...button}>
                  {button.title}
                </Button>
              ))}
            </div>
        ...
        <div className="hidden justify-self-end lg:block">
          {buttons.map((button, index) => (
            <Button key={index} className="px-6 py-2" style={{ backgroundColor: '#781E26' }} {...button}>
              {button.title}
            </Button>
          ))}
        </div>
